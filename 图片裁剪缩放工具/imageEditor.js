...
imageEditor.load = function(W3CEvent) {
    ...
    // appropriate xADSImageEditor class name
    var forms = xADS.getElementsByClassName('ADSImageEditor', 'FORM');
    ...
    for( var i=0 ; i < forms.length ; i++ ) {
        ...
        xADS.addEvent(images[0],'click',imageEditor.imageClick);
...
imageEditor.imageClick = function(W3CEvent) {
    ...
    */
    ...
    xADS.setStyle(cropResizeHandle,{
        'position':'absolute',
        'background':'transparent url(interface/handles.gif) no-repeat 0 0'
    });
    ...
    xADS.addEvent(resizeHandle,'mouseover',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'0px -29px'});
    });
    xADS.addEvent(resizeHandle,'mouseout',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'0px 0px'});
    });
    
    // Crop handle rollovers
    xADS.addEvent(cropResizeHandle,'mouseover',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'0px -29px'});
    });
    xADS.addEvent(cropResizeHandle,'mouseout',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'0px 0px'});
    });
    
    // Save handle rollovers
    xADS.addEvent(saveHandle,'mouseover',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'-40px -29px'});
    });
    xADS.addEvent(saveHandle,'mouseout',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'-40px 0px'});
    });
    
    // Cancel handle rollovers
    xADS.addEvent(cancelHandle,'mouseover',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'-29px -40px'});
    });
    xADS.addEvent(cancelHandle,'mouseout',function(W3CEvent) {
        xADS.setStyle(this,{'background-position':'-29px -11px'});
    });

    /* Mouse events for the handles */

    // Start the image resizing event flow
    xADS.addEvent(resizeHandle,'mousedown',imageEditor.resizeMouseDown);

    // Start the crop area drag event flow   
    xADS.addEvent(cropArea,'mousedown',imageEditor.cropMouseDown);
    
    // Start the crop area resize event flow
    xADS.addEvent(cropResizeHandle,'mousedown',function(W3CEvent) {
        imageEditor.info.resizeCropArea = true;
    });

    // Prevent the save handle from starting the crop drag flow
    xADS.addEvent(saveHandle,'mousedown',function(W3CEvent) {
            xADS.stopPropagation(W3CEvent);
    });
    // Save the image on click of the save handle or dblckick of the crop area
    xADS.addEvent(saveHandle,'click',imageEditor.saveClick);
    xADS.addEvent(cropArea,'dblclick',imageEditor.saveClick);

    // Prevent the cancel handle from starting the crop drag flow
    xADS.addEvent(cancelHandle,'mousedown',function(W3CEvent) {
            xADS.stopPropagation(W3CEvent);
    });
    // Cancel the changes on click
    xADS.addEvent(cancelHandle,'click',imageEditor.cancelClick);
    
    // Resize the backdrop if the window size changes 
    xADS.addEvent(window,'resize',function(W3CEvent) {
        var windowSize = getWindowSize();
        setNumericStyle(backdrop,{
            'left':0,
            'top':0,
...
imageEditor.resizeMouseDown = function(W3CEvent) {
    ...
    imageEditor.info.pointerStart = xADS.getPointerPositionInDocument(W3CEvent)
    ...
    xADS.addEvent(document,'mousemove',imageEditor.resizeMouseMove);
    xADS.addEvent(document,'mouseup',imageEditor.resizeMouseUp);
...
imageEditor.resizeMouseMove = function (W3CEvent) {
    ...
    var pointer = xADS.getPointerPositionInDocument(W3CEvent);
    ...
    if(xADS.getEventObject(W3CEvent).shiftKey) {
...
imageEditor.resizeMouseUp = function (W3CEvent) {
    ...
    xADS.removeEvent(document,'mousemove',imageEditor.resizeMouseMove);
    xADS.removeEvent(document,'mouseup',imageEditor.resizeMouseUp);
...
imageEditor.cropMouseDown = function(W3CEvent) {
    ...
    xADS.addEvent(document,'mousemove', imageEditor.cropMouseMove);
    xADS.addEvent(document,'mouseup', imageEditor.cropMouseUp);
...
imageEditor.cropMouseUp = function(W3CEvent) {
    ...
    var eventObject = xADS.getEventObject(W3CEvent);
    imageEditor.info.resizeCropArea = false;
    
    xADS.removeEvent(document,'mousemove', imageEditor.cropMouseMove);
    xADS.removeEvent(document,'mouseup', imageEditor.cropMouseUp);
...
window['xADS']['imageEditor'] = imageEditor;


})();

// Add the load event to the window object using the xADS.addLoadEvent()
// method because this page may contain a lot of images
xADS.addLoadEvent(xADS.imageEditor.load);
